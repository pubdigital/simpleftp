 ...
 * sd: socket descriptor for control connection
 * datafd: socket descriptor for data connection 
 * file_name: file name to get from the server
 **/
void get(int sd, int datafd, char *file_name) {
 ...
 **/
void put(int sd, int datafd, char *file_name) {
    char desc[BUFSIZE], buffer[BUFSIZE];
    int f_size, recv_s, r_size = BUFSIZE;
    int bread;
    FILE *file;
    ...
    while(1) {
        ...
        if (bread > 0) {
            ...
            sleep(1); // important delay for avoid problems 
        }
 ...
 * datafd: socket descriptor for data connection 
 **/
void operate(int sd, int datafd) {
    char *input, *op, *param;
...
int main (int argc, char *argv[]) {
    ...
    if((hostName = gethostbyname(argv[1])) == NULL) {
        perror("Error in function gethostbyname");
        exit(1);
    }
