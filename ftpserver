 ...
 **/
void retr(int controlfd, int datafd, char *file_path) {
    FILE *file;
    int bRead;
    long fsize;
    char buffer[BUFSIZE];
    ...
    if(file == NULL) {
        ...
        send_ans(controlfd, MSG_550, file_path);
        return;
    }
    ...
    send_ans(controlfd, MSG_150, file_path, fsize);
...
void operate(int controlfd, int datafd) {
    ...
    while (true) {
        ...
        if (!recv_cmd(controlfd, op, param)) {
            exit(1);
        }

        if (strcmp(op, "RETR") == 0) {
            retr(controlfd, datafd, param);
        } else if(strcmp(op, "STOR") == 0) {
            stor(controlfd, datafd, param);
        } else if(strcmp(op, "QUIT") == 0) {
            // send goodbye and close connection
            send_ans(controlfd, MSG_221);
            close(controlfd);
            close(datafd);
            break;
        } else {
...
/**
 * function: split the string that contains IP address and port numbers,
...
int setup_data_connection(int *fd, char *client_ip, int client_port, int server_port) {    
    ...
    printf("server_port sin htons: %d\n", server_port);
    printf("server_port con htons: %d\n", htons(server_port));
    printf("tempAddr.sin_port: %d\n", tempAddr.sin_port);
    printf("tempAddr.sin_port con ntohs: %d\n", ntohs(tempAddr.sin_port));
     **/
...
int main (int argc, char *argv[]) {
    ...
    serverAddr.sin_port = htons(portNumber); /* to make sure that numbers are stored in memory in network byte order */
    serverAddr.sin_addr.s_addr = htonl(INADDR_ANY); /* with INADDR_ANY the socket binds to all the local interfaces */
    memset(&(serverAddr.sin_zero), '\0', 8);

    // create server socket and check errors
    listenfd = socket(AF_INET, SOCK_STREAM, 0); /* creates the socket and returns its file descriptor */
    if (listenfd < 0) {
    ...
    while (true) {
    	...
    	if((clientsock = accept(listenfd, (struct sockaddr *) &clientAddr, (socklen_t *) &sin_size)) < 0) {
        ...
        if(authenticate(clientsock) != true) {
            ...
            if (pid == 0) {
                ...
                close(listenfd);          // se cierra el socket inicial
