...
int main (int argc, char *argv[]) {
    ...
    int serverSocket, fileDescriptor, serverPort;
    ...
    while (true) {
        ...
        if ((fileDescriptor = accept(serverSocket, (struct sockaddr *)&clientAddr,(socklen_t *)&sin_size)) == -1) {
        ...
        if(!authenticate(fileDescriptor)) close(fileDescriptor);
            ...
            if (!fork()) {
                ...
                if((setup_data_connection(&dataSocket, client_ip, client_port, serverPort)) < 0) {
                   break;
                }
